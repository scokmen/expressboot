...
import { Options } from "../../../../src/ExpressBoot";
...
describe("options specs", () => {
    ...
    it("should annotate given method with handler tags", () => {
        ...
        expect(metadataList[0].httpMethod).to.equal("options");
    ...
    it("should throw error when path is not a string", () => {
        ...
        expect(fn).to.throws(`An error occurred annotating Mock.Handler(...) as 'options' handler. Error: Path cannot be empty`);
    });
    ...
    it("should throw error when method annotated more than once", () => {
        ...
        expect(fn).to.throws(`An error occurred annotating Mock.Handler(...) as 'options' handler. Error: Class Mock.Handler(...) already annotated as handler`);
    });
});
